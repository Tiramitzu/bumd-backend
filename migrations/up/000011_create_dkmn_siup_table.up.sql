CREATE TABLE dkmn_siup (
    ...
    CONSTRAINT dkmn_siup_pkey PRIMARY KEY (id)
);

-- Add foreign key constraint to bumd table
ALTER TABLE dkmn_siup 
ADD CONSTRAINT fk_dkmn_siup_id_bumd 
FOREIGN KEY (id_bumd) REFERENCES bumd(id);

-- Add indexes for better performance
CREATE INDEX idx_dkmn_siup_id_bumd 
    ON dkmn_siup(id_bumd);

CREATE INDEX idx_dkmn_siup_nomor 
    ON dkmn_siup(nomor);

CREATE INDEX idx_dkmn_siup_tanggal 
    ON dkmn_siup(tanggal);

CREATE INDEX idx_dkmn_siup_masa_berlaku 
    ON dkmn_siup(masa_berlaku);

CREATE INDEX idx_dkmn_siup_deleted_at 
    ON dkmn_siup(deleted_at);