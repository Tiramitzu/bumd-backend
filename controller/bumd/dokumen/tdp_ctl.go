...
func (c *TdpController) Index(
	...
	if err != nil {
		return r, totalCount, pageCount, utils.RequestError{
			...
			Message: "gagal mengambil data TDP. - " + err.Error(),
		}
	}
	...
	if err != nil {
		return r, totalCount, pageCount, utils.RequestError{
			Code:    fasthttp.StatusInternalServerError,
			Message: "gagal menghitung total data TDP. - " + err.Error(),
		}
	}
	...
	for rows.Next() {
		...
		if err != nil {
			return r, totalCount, pageCount, utils.RequestError{
				Code:    fasthttp.StatusInternalServerError,
				Message: "gagal memindahkan data TDP. - " + err.Error(),
			}
		}
		r = append(r, m)
	}
...
func (c *TdpController) Update(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd, id uuid.UUID, payload *dokumen.TdpForm) (r bool, err error) {
	...
	if payload.MasaBerlaku != nil {
		...
		q = `UPDATE trn_tdp SET masa_berlaku_tdp='' WHERE id_tdp=$1 AND id_bumd=$2`
...
func (c *TdpController) Delete(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd, id uuid.UUID) (r bool, err error) {
	...
	if err != nil {
		return false, utils.RequestError{
			Code:    fasthttp.StatusInternalServerError,
			Message: "gagal menghapus data TDP. - " + err.Error(),
		}
	}
	return true, err
}
