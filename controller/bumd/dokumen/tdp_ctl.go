...
func (c *TdpController) Create(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd int, payload *dokumen.TdpForm) (r bool, err error) {
	...
	q := `
	INSERT INTO dkmn_tdp (nomor, instansi_pemberi, tanggal, kualifikasi, klasifikasi, masa_berlaku, file, id_bumd, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
	`
...
func (c *TdpController) Update(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd int, payload *dokumen.TdpForm) (r bool, err error) {
	claims := user.Claims.(jwt.MapClaims)
	idUser := int(claims["id_user"].(float64))
	idBumdClaims := int(claims["id_bumd"].(float64))
