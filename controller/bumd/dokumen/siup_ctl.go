...
func NewSiupController(pgxConn *pgxpool.Pool) *SiupController {
	return &SiupController{pgxConn: pgxConn}
}
...
func (c *SiupController) Index(
	...
	qCount := `SELECT COALESCE(COUNT(*), 0) FROM dkmn_siup WHERE deleted_by = 0 AND id_bumd = $1`
	q := `
	...
	if err != nil {
		return r, totalCount, pageCount, fmt.Errorf("gagal menghitung total data SIUP: %w", err)
	}
	...
	if err != nil {
		return r, totalCount, pageCount, fmt.Errorf("gagal mengambil data SIUP: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		...
		if err != nil {
			return r, totalCount, pageCount, fmt.Errorf("gagal memindahkan data SIUP: %w", err)
		}
		r = append(r, m)
	}
...
func (c *SiupController) View(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd, id int) (r dokumen.SiupModel, err error) {
	claims := user.Claims.(jwt.MapClaims)
	idBumdClaims := int(claims["id_bumd"].(float64))
	...
	if err != nil {
		...
		return r, fmt.Errorf("gagal mengambil data SIUP: %w", err)
	}

	return r, err
}

func (c *SiupController) Create(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd int, payload *dokumen.SiupForm) (r bool, err error) {
	claims := user.Claims.(jwt.MapClaims)
...
func (c *SiupController) Update(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd, id int, payload *dokumen.SiupForm) (r bool, err error) {
	claims := user.Claims.(jwt.MapClaims)
	idUser := int(claims["id_user"].(float64))
	idBumdClaims := int(claims["id_bumd"].(float64))
...
func (c *SiupController) Delete(fCtx *fasthttp.RequestCtx, user *jwt.Token, idBumd, id int) (r bool, err error) {
	claims := user.Claims.(jwt.MapClaims)
	idUser := int(claims["id_user"].(float64))
	idBumdClaims := int(claims["id_bumd"].(float64))
